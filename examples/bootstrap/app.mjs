...
const REQ = Middleware.get.map(R.prop('req'));
const QUERY = Middleware.get.map(R.prop('query'));
const error = (code, message) => Middleware.lift(
...
.use(App.do(function*(next) {
  ...
  yield Middleware.modify(R.assoc('query', query));
  return yield next;
}))
...
.use(App.do(function*(next) {
  ...
  const connection = yield Middleware.lift(
